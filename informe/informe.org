...
** INPROGRESS Trabajo relacionado
...
*** Obtención del Dataset
    ...
    que se abuse del servicio[fn:10]. Según la investigación
    realizada, es posible realizar una llamada a la API por segundo,
    dando la capacidad de 4 búsquedas completas, o 6000 tweets por
    minuto en el mejor caso.
    ...
    - Una vez a la semana se obtendrán las noticias más importantes
      del día[fn:11], y luego se buscará en Twitter los tweets que
      mencionen a esas noticias.
...
[fn:10] https://dev.twitter.com/docs/rate-limiting#search
[fn:11] Por ejemplo, esta URL retorna las noticias más relevantes en la edición en inglés de Google News: https://ajax.googleapis.com/ajax/services/search/news?v=1.0&topic=h
...
** Herramientas a utilizar
   ...
   Se utilizará el lenguaje =Python= [fn:12], dada la experiencia
   previa, la flexibilidad del lenguaje para realizar prototipos
   rápidos y la gran cantidad de librerías existentes para manipular
   datos y conectarse a distintos servicios. En particular:

   - =Twython= [fn:13], una librería para realizar búsquedas y acceder
     fácilmente a la información que provee Twitter. Con esta librería
     se realizaron algunas búsquedas para probar la factibilidad de la
     metodología de obtención de datos descrita en esta sección.
   - Para acceder a distintos sitios web para obtener documentos, la
     librería =mechanize= [fn:14] emula un Web Browser en =python=, y
     para analizar y procesar las páginas HTML de cada sitio,
     =BeautifulSoup= [fn:15] cumple tal objetivo. Para almacenar y
     empaquetar datos, =simplejson= [fn:16] permite el manejo de JSON
   ...
   conocido como /NoSQL/). Se han considerado =MongoDB= [fn:17] y
   =Redis= [fn:18] para este propósito, con las respectivas librerías
   en =python= que se encargan de crear una interfaz entre el lenguaje
   de programación y el software de base de datos.

   La librería de minería de datos =Orange= [fn:19] ofrece varias
   funcionalidades como Clasificación, Clustering, Reglas de
   Asociación, entre otras. No se ha visto en detalle las capacidades
   y utilidad de esta herramienta, pero queda en consideración su uso
   en el futuro.
   ...
   =Django= [fn:20] y =Flask= [fn:21] son buenas alternativas.
   ...
   [fn:22] y el servicio GitHub[fn:23] para almacenarlo
   remotamente. Tanto el trabajo adelantado hasta la fecha y este
   mismo informe están en este servicio[fn:24]. 

   Para el desarrollo de este informe y los siguientes, se utilizó y
   utilizará =GNU Emacs= [fn:25] y su extensión =org-mode= [fn:26] que
   permite escribir documentos rápidamente y generar el archivo $\LaTeX$
   (y correspondientemente, PDF) equivalente.

[fn:12] http://www.python.org/
[fn:13] https://github.com/ryanmcgrath/twython
[fn:14] http://wwwsearch.sourceforge.net/mechanize/
[fn:15] http://www.crummy.com/software/BeautifulSoup/
[fn:16] https://github.com/simplejson/simplejson
[fn:17] http://www.mongodb.org/
[fn:18] http://redis.io/
[fn:19] http://orange.biolab.si/
[fn:20] https://www.djangoproject.com/
[fn:21] http://flask.pocoo.org/
[fn:22] http://git-scm.com/
[fn:23] http://www.github.com/
[fn:24] https://github.com/mquezadav/cc6909
[fn:25] http://www.gnu.org/software/emacs/
[fn:26] http://orgmode.org/
